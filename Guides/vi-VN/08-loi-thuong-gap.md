# Lỗi thường gặp

## App báo không thấy game

Chọn thư mục ngay phía trên `Client`, ví dụ:

```text
E:\Games\Wuthering Waves\Wuthering Waves Game
```

Không chọn `...\Client`, `...\Client\Content\Paks` hoặc thư mục launcher bên
ngoài. Bấm **Kiểm tra file** sau khi chọn.

## Cài xong vẫn còn tiếng Anh

- Đặt ngôn ngữ chữ trong game là **English**.
- Khởi chạy bằng **DirectX 11**.
- Mở lại app và xem trạng thái hậu kiểm có báo đủ PAK/SIG/font/loader không.
- Nếu chỉ riêng cốt truyện/kỹ năng 3.6 còn EN, bảo đảm đang dùng bản 2.0.0 mới.

## Chữ có dấu thành ô vuông

Cài lại với tùy chọn **Cài kèm font tiếng Việt**. Nếu đang dùng font/mod ngoài,
gỡ nó để tránh ghi đè font của bản Việt hóa.

## App báo có mod hoặc bản Việt hóa khác

Không cài chồng. Gỡ bản cũ bằng công cụ đã dùng để cài nó, hoặc Verify/Repair
game, rồi chạy lại **Kiểm tra file** trước khi cài VHWuWa.

## Game văng

1. Gỡ Việt hóa và thử game gốc.
2. Nếu game gốc vẫn văng, Verify/Repair và kiểm tra driver/cấu hình máy.
3. Nếu chỉ văng khi cài, gửi ảnh lỗi và danh sách file app báo đã cài.

## Muốn trả game về nguyên bản

Tắt game → **Cài Việt hóa** → **Gỡ Việt hóa**. Sau đó dùng Verify/Repair nếu
trước đây từng cài thêm mod bằng công cụ khác.
