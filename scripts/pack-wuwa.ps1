...
# pak/font trong Content\Paks\~WuWaMods\ (file mới → Copy)
$modsDir = Join-Path $Win64Dir "..\..\Content\Paks\~WuWaMods"
if (Test-Path $modsDir) {
    foreach ($f in Get-ChildItem $modsDir -File -Recurse) {
        $rel = "~WuWaMods/" + $f.FullName.Substring($modsDir.Length).TrimStart('\', '/').Replace('\', '/')
